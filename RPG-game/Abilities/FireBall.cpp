...
FireBall::FireBall(sf::Vector2f playerPostion,std::string texturePath, NumberOfXYParts&& spriteNumbers, sf::RenderWindow* window, size_t FireRat,short speed)
    :Ability(playerPostion,texturePath, spriteNumbers, window, speed, FireRat), motionState(0)
{
...
FireBall::FireBall(FireBall& fireball2) :Ability(fireball2.playerPostion, fireball2.texturePath, fireball2.spriteNumbers, fireball2.window, fireball2.speed, fireball2.FireRat), motionState(0)
{
    abilitySpirit->setTextureRect(sf::IntRect(sf::Vector2i((motionState + WALKING) * WIDTH, FiredDirection * HEIGHT), sf::Vector2i(WIDTH, HEIGHT)));
}
...
void FireBall::Update(const sf::Vector2f& Target)
{
    ...
    sf::Vector2f normalizedDir = (Target - abilitySpirit->getPosition()).normalized().componentWiseMul({ 30.f,30.f });
